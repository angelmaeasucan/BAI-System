...
class Sale(Entity):
    ...
    def delete(self):
        if self.__id is not None:
            self._data[:] = [s for s in self._data if s['id'] != self.__id]
...
class Bill(Entity):
    ...
    def delete(self):
        if self.__id is not None:
            self._data[:] = [b for b in self._data if b['id'] != self.__id]
...
class Activity(Entity):
    ...
    def delete(self):
        # Activities are logs, deletion not implemented
        pass
...
class POSApp:
    ...
    def setup_routes(self):
        ...
        def admin_dashboard():
            return render_template('admin/dashboard.html', **Dashboard.get_metrics())

        @self.__app.route('/cashier')
        def cashier_dashboard():
            return render_template('cashier/dashboard.html', **Dashboard.get_metrics())
        ...
        def users():
            user_list = [dict(user) for user in User._data]
